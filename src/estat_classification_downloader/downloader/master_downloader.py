...
@pa.check_types
def download_estat_classification_master(
    classification_type: str, revision_code: str
) -> DataFrame[EStatClassificationMasterSchema]:
    params = {
    ...
    if not response.text.strip():
        raise EStatDownloadError("Received empty response from E-Stat server")

    with io.StringIO(response.text) as csv_io:
        df = pd.read_csv(csv_io, skiprows=[0, 1, 2], dtype=str, names=["code", "code_name", "desc"])

    return DataFrame[EStatClassificationMasterSchema](df)
